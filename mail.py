...
def send_message(content, task_conf):
    with smtplib.SMTP(SMTP_IP) as smtp:
        ...
        html = f"""\
            ...
            <body>
                <p>Здравствуйте!<br>
                Ваш запрос на кластере GP FDEV достиг лимита по правилу '{task_conf["email_rule_text"]} {task_conf["limit"]}'. Пока это предупреждение, но в дальнейшем запрос может быть автоматически прерван, а учетная запись заблокирована.<br>
                </p>
                <p>
